	...
	while(a<b)
	{
   		for( i=0,j=5; i<=10, j<=13; i++,j++)
		{
	   		for(k=0;k<20;k++)
			{
	   		...
	   		if(*GPIOA_ODR & 1<<i && *GPIOB_ODR & 1<<j)				// condition checking
        		{
                		*GPIOA_ODR = *GPIOA_ODR & ~(1<<i);	// LED OFF
				*GPIOB_ODR = *GPIOB_ODR & ~(1<<j);	// LED OFF
        		}
        		else
        		{
                		*GPIOA_ODR = *GPIOA_ODR | (1<<i);	// LED ON
				*GPIOB_ODR = *GPIOB_ODR | (1<<j);	// LED ON
        		}
		}
		a++;
		a++;
